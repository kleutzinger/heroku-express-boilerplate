...
function partitionRowsBySet(rows) {
  ...
  for (const game of row_copy) {
    ...
    if (!_.has(games_by_codes, codes)) {
      games_by_codes[codes] = { chron: [], codes, part: [] };
    }
  ...
  _.forEachRight(row_copy, (game, idx) => {
    ...
    if (_last.codes_sorted !== game.codes_sorted) {
    ...
    // games are probably in the same set?
    // (same players, similar time)
    cur_set.push(game);
  });
  if (!_.isEmpty(cur_set)) {
    partitions.push(cur_set);
