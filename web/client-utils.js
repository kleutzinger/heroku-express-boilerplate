...
function drawIcons(ctx, imageURL, done_cb = null) {
  ...
  function allLoaded() {
    ...
    ctx.drawImage(images[0], 0, 0); // draw background
    ctx.drawImage(images[1], 100, 0); // draw foreground
    ctx.drawImage(images[2], 300, 0); // draw foreground
...
function makeImg(ctx, image_urls, done_cb = null) {
  // image paths: [stage, char0, char1...]
  drawIcons(ctx, image_urls, done_cb);
...
function drawMetaToScreen(metadata) {
  ...
  makeImg(ctx, icon_paths, canvasToBody);
}
...
document.addEventListener('DOMContentLoaded', function(event) {
  let canvas = document.querySelector('canvas');
  // makeImg(canvas, falcons, canvasToBody);
  drawMetaToScreen(example_meta);
});
...
const example_meta = {
  slp_metadata : {
    players   : {
      '0' : {
        names      : { code: 'QUAS#458', netplay: 'Quas' },
        characters : { '1': 813 }
      },
      '1' : {
        names      : { code: 'KB#795', netplay: 'KevbotSSBM' },
        characters : { '3': 813 }
      }
    },
    startAt   : '2020-09-04T18:31:00Z',
    playedOn  : 'dolphin',
    lastFrame : 687
  },
  slp_settings : {
